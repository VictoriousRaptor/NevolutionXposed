...
public class WeChatDecorator extends NevoDecoratorService {
	...
	public static class WeChatSystemUIDecorator extends NevoDecoratorService.SystemUIDecorator implements HookSupport {
		...
		@Override public Decorating onNotificationPosted(final StatusBarNotification evolving) {
			...
			if (overrideContentView) {
				if (overridedContentView(n) == null) n.contentView = overrideContentView(n, new RemoteViews(BuildConfig.APPLICATION_ID, R.layout.wechat_notifition_layout));
		...
		public void onNotificationRemoved(final String key, final int reason) {
	...
	private void recastNotification(final String key, final ModifyStatusBarNotification... modifies) {
