...
public class WeChatDecorator extends NevoDecoratorService {
	public static class WeChatLocalDecorator extends NevoDecoratorService.LocalDecorator implements HookSupport {
		...
		@Override public void onCreate(SharedPreferences pref) {
			...
			mMessagingBuilder = new MessagingBuilder(getAppContext(), getPackageContext(), decorator::modifyNotification);		// Must be called after loadPreferences().
		...
		@Override public Decorating apply(NotificationManager nm, String tag, int id, Notification n) {
			...
			MessagingStyle messaging = mMessagingBuilder.buildFromExtender(conversation, id, n, title, getArchivedNotifications0(id)); // build message from android auto
			if (messaging == null)	// EXTRA_TEXT will be written in buildFromArchive()
				messaging = mMessagingBuilder.buildFromArchive(conversation, n, title, getArchivedNotifications0(id));
		...
		private void reviveNotificationAfterChannelDeletion(final int id) {
			...
			decorator.modifyNotification(id, n -> {
	...
	public static class WeChatSystemUIDecorator extends NevoDecoratorService.SystemUIDecorator implements HookSupport {
		...
		@Override public Decorating onNotificationPosted(final StatusBarNotification evolving) {
			...
			// 	}
			// }
			// if (content_text == null) return Decorating.Unprocessed;
		...
		public void onNotificationRemoved(final String id, final int reason) {
			Log.d(TAG, "onNotificationRemoved(" + id + ", " + reason + ")");
			// if (reason == REASON_APP_CANCEL) {		// Only if "Removal-Aware" of Nevolution is activated
			// 	Log.d(TAG, "Cancel notification: " + id);
			// 	// cancelNotification(id);	// Will cancel all notifications evolved from this original key, thus trigger the "else" branch below
			...
			// 	// mHandler.post(() -> reviveNotificationAfterChannelDeletion(id));
		...
		@Override public void onCreate(SharedPreferences pref) {
			...
			// mMessagingBuilder = new MessagingBuilder(getAppContext(), getPackageContext(), decorator::recastNotification);		// Must be called after loadPreferences().
		}
	...
	private void modifyNotification(final int id, final ModifyNotification... modifies) {
		if (hasArchivedNotifications0(id)) {
			Notification n = getArchivedNotification0(id);
